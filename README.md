...
Sprint 12 shipped the behavioral profile infrastructure: per-identity rolling windows, z-score deviation on connection rate and byte volume, novelty scoring on destination set, `GET /profile`. This is validated against synthetic test scenarios (burst traffic, novel destinations) — not yet against production traffic. The run-stage goal is running it against 30+ days of real production traffic to confirm the baseline model is stable enough to surface real anomalies without false-positive noise. The SPIFFE Firewall handles known-bad. Behavioral fingerprinting handles unknown-bad — the zero-day, the compromised credential, the agent that went rogue.

**6. Capture inside isolated compute — microVMs**
The reflector's uprobe-based capture reads plaintext where the TLS library call happens inside a process the host kernel can see. That breaks down for workloads running inside a microVM (e.g. Firecracker) — the TLS call happens in a different kernel the host can't reach. Closing this requires an in-guest observer with a way to get a verifiable identity without a secret baked into every image. Currently spiking a CID-vouched identity pattern: the microVM host assigns each guest an unforgeable vsock context ID (CID) at launch; a host-side broker reads the connecting CID and vouches to the identity server for SVID issuance — zero-touch, no per-image secret. This is prototype-stage, gated on two go/no-go spikes (does the uprobe fire inside a guest kernel at all; does the CID-vouched attestation flow hold up end to end, including rejecting a spoofed CID) before any production commitment.
